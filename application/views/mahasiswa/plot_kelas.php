                        ...
                        <hr>
                            ...
                            <select multiple="multiple" size="10" name="duallistbox_demo1[]" title="duallistbox_demo1[]">
                                <?php foreach($data_master as $row){ ?>
                                    <option value="<?php echo $row->nim; ?>" <?php if($row->id_kelas==$id_kelas){ echo "selected";} ?>><?php echo $row->nim; ?> - <?php echo $row->nama; ?></option>
                                <?php } ?>
                            </select>
                            <br>
                            ...
                            $("#demoform").submit(function() {
                                $.post("<?php echo $action; ?>",
                                    ...
                                    function(data,status){
                                        alert("Data: " + data + "\nStatus: " + status);
                                        window.location.href = "<?php echo base_url('baak/mahasiswa/kelas') ?>";
                                    });
