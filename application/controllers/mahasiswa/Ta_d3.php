...
class Ta_d3 extends CI_Controller {
    ...
    public function index(){
        ...
        if($v_mahasiswa->nama_jenjang_pendidikan != 'D3'){
            $text = $this->alert->danger('You do not have access');
			$this->session->set_flashdata('message', $text);
			redirect("welcome/dashboard");
        }
    ...
    public function json_list_periode_d3(){
        ...
        echo $this->Baak_dt->json_list_periode_d3_mhs();
    }

    public function daftar($periode){
        $rule_date=$this->Master_model->master_get(['periode'=>$periode],'ta_d3_list');
        ...
        $now = strtotime(date('Y-m-d'));
                if($start < $now and $now <$close and $rule_date->status=='open' ){
                    ...
                    $this->session->set_flashdata('message', $text);
                    redirect("mahasiswa/ta_d3/");
                }

        $rules=$this->Master_model->master_result(['periode'=>$periode],'ta_d3_rule');
        if($rules){
            $rule=$this->Master_model->master_result(['periode'=>$periode,'nim'=>$this->session->userdata('username')],'ta_d3_mhs_rule');
            if(!$rule){
                $text = $this->alert->warning('anda belum melakukan pembayaran');
                $this->session->set_flashdata('message', $text);
                redirect("mahasiswa/ta_d3/cek_rule/".$periode);
            }
        }

        $this->load->model('form/Mhs_f', 'mahasiswa_form');

        $data_master=$this->mahasiswa_form->daftar_ta_d3($periode);
        $magang_mhs_list=$this->Master_model->master_get(['nim'=>$this->session->userdata('username')],'magang_mhs_list');
        if($magang_mhs_list){
            $historisidang['judul']=$magang_mhs_list->judul;
        }else{
            $historisidang['judul']='';
        }
    ...
    public function daftar_ta_d3_action($periode){
        // $nama_pt = $this->input->post('nama_pt');
        $this->form_validation->set_rules('judul','judul','trim|required|xss_clean');
        $this->form_validation->set_error_delimiters('<span class="text-danger">', '</span>');
		if (($this->form_validation->run() == TRUE)   ) {
            $data_masters=($this->Master_model->master_get(['periode' => $periode,'nim' => $this->session->userdata('username')],'ta_d3_mhs_list'));
            if($data_masters){
                $text = $this->alert->warning('you have registered in : '.$data_masters->judul);
                $this->session->set_flashdata('message', $text);
                redirect(site_url('mahasiswa/ta_d3/'));
            }
            ...
            $this->Master_model->insert_query('ta_d3_mhs_list',$data);
    ...
    public function cek_rule($periode){
        $rule=$this->Master_model->master_result(['periode'=>$periode],'ta_d3_rule');
        if($rule){
            foreach($rule as $row){
                ...
                if($result->status_code=='000'){
                    ...
                    if($data->status_bayar=='sudah'){
                        ...
                        $this->Master_model->insert_query('ta_d3_mhs_rule',$data);

                        $text = $this->alert->success('rule ok');
                        $this->session->set_flashdata('message', $text);
                        redirect("mahasiswa/ta_d3/daftar/".$periode);
                    }
                }
    ...
    public function dashboard($periode){
        $data_masters=($this->Master_model->master_get(['nim' => $this->session->userdata('username')],'v_ta_d3_mhs_list'));
        // $data_masters=($this->Master_model->master_get(['periode' => $periode,'nim' => $this->session->userdata('username')],'v_ta_d3_mhs_list'));
        if($data_masters){
           
            $this->load->model('Baak_model');
            $nim = $this->session->userdata('username');

            $jadwal_sidang=($this->Mahasiswa_model->jadwal_sidang_ta_d3($periode,$data_masters->kode_prodi));
            $jadwal_yudisium=($this->Mahasiswa_model->jadwal_yudisium($periode,$data_masters->kode_prodi));
            $history_sidang=$this->Master_model->master_result(['nim'=>$this->session->userdata('username')],'v_ta_d3_mhs_sidang');
            ...
            //print_r($kurikulum[0]);
            $this->header();
            $this->load->view('mahasiswa/ta_d3/dashboard',
                [
    ...
    public function pendaftar_sidang_ta_d3($id_trx){
        $rule_date=$this->Master_model->master_get(['id_trx'=>$id_trx],'ta_d3_jadwal_sidang');
        if(!$rule_date){
        ...
        $cek_rule_byid=$this->Master_model->master_get(['nim'=>$this->session->userdata('username'),'id_rule'=>$rule_date->id_rule],'ta_d3_mhs_rule');
        if(!$cek_rule_byid){
            $this->cek_rule_byid($rule_date->periode,$rule_date->id_rule,'mahasiswa/ta_d3/dashboard/'.$rule_date->periode);
        }

        $this->load->model('form/Mhs_f', 'mahasiswa_form');
        
        $historisidang=($this->Master_model->master_get(['id_sidang'=>$id_trx,'nim'=>$this->session->userdata('username')],'ta_d3_mhs_sidang'));
        $data_master=$this->mahasiswa_form->pendaftar_sidang_ta_d3($id_trx,$rule_date);
        ...
        if($historisidang){
            ...
            $historisidang=get_object_vars($this->Master_model->master_get(['periode'=>$rule_date->periode,'nim'=>$this->session->userdata('username')],'ta_d3_mhs_list'));
    ...
    public function pendaftar_sidang_ta_d3_action($id_trx){
        $rule_date=$this->Master_model->master_get(['id_trx'=>$id_trx],'ta_d3_jadwal_sidang');
        if(!$rule_date){
            $text = $this->alert->warning('schedule not found');
                    $this->session->set_flashdata('message', $text);
        ...
        $ta_d3_mhs_list=$this->Master_model->master_get(['periode'=>$rule_date->periode,'nim'=>$this->session->userdata('username')],'ta_d3_mhs_list');
        ...
        if($rule_date->sidang=='hasil'){
            if(!in_array($ta_d3_mhs_list->progres,['bimbingan','daftar sidang hasil','penilaian sidang proposal']) ){
                $text = $this->alert->warning('You do not have access. you progres : '.$ta_d3_mhs_list->progres);
                $this->session->set_flashdata('message', $text);
                redirect("mahasiswa/ta_d3/dashboard/".$ta_d3_mhs_list->periode); 
            }
        }elseif($rule_date->sidang=='ta'){
            if(!in_array($ta_d3_mhs_list->progres,['daftar sidang TA','penilaian sidang hasil']) ){
                $text = $this->alert->warning('You do not have access. you progres : '.$ta_d3_mhs_list->progres);
                $this->session->set_flashdata('message', $text);
                redirect("mahasiswa/ta_d3/dashboard/".$ta_d3_mhs_list->periode); 
            }
		...
		if (($this->form_validation->run() == TRUE)   ) {
            ...
            if ( ! $this->upload->do_upload('berkas'))
            {
                $text = $this->alert->danger('Data failed Added. Info :'.$this->upload->display_errors());
                    $this->session->set_flashdata('message', $text);
                    return redirect("mahasiswa/ta_d3/pendaftar_sidang_ta_d3/".$id_trx);
            }
            else
            {

                $ta_d3_mhs_sidang=$this->Master_model->master_get(['id_sidang'=>$id_trx,'periode'=>$rule_date->periode,'nim'=>$this->session->userdata('username')],'ta_d3_mhs_sidang');
                if($ta_d3_mhs_sidang){
                    $this->Master_model->insert_history('update','ta_d3_mhs_sidang',json_encode($ta_d3_mhs_sidang));
                    ...
                    if($ta_d3_mhs_sidang->progres=='ditolak'){
                        $data['progres']='daftar';
                    }

                    $this->Master_model->update_query(['id_sidang'=>$ta_d3_mhs_sidang->id_trx,'periode'=>$rule_date->periode,'nim'=>$this->session->userdata('username')], $data, 'ta_d3_mhs_sidang');
                    ...
                    $data=array(
                        ...
                        'id_list'=>$ta_d3_mhs_list->id_trx,
                    );
                    $this->Master_model->insert_query('ta_d3_mhs_sidang',$data);
                }
                    ...
                    $this->Master_model->update_query(['periode'=>$rule_date->periode,'nim'=>$this->session->userdata('username')], $data, 'ta_d3_mhs_list');
            ...
            $this->pendaftar_sidang_ta_d3($id_trx);
        }

    }

    public function pendaftar_yudisium_ta_d3($id_trx){
        ...
        $now = strtotime(date('Y-m-d'));
                ...
                if(!in_array($ta_d3_mhs_list->progres,['daftar sidang TA','penilaian sidang TA','daftar yudisium']) ){
                    $text = $this->alert->warning('You do not have access. you progres : '.$ta_d3_mhs_list->progres);
                    $this->session->set_flashdata('message', $text);
                    redirect("mahasiswa/ta_d3/dashboard/".$ta_d3_mhs_list->periode); 
                }
        ...
        $data_master=$this->mahasiswa_form->pendaftar_yudisium_ta_d3($id_trx,$rule_date);
    ...
    public function pendaftar_yudisium_ta_d3_action($id_trx){
        $rule_date=$this->Master_model->master_get(['id'=>$id_trx],'yudisium');
        if(!$rule_date){
            $text = $this->alert->warning('schedule not found');
                    $this->session->set_flashdata('message', $text);
		...
		if (($this->form_validation->run() == TRUE)   ) {
            ...
            if ( ! $this->upload->do_upload('berkas'))
            {
                $text = $this->alert->danger('Data failed Added. Info :'.$this->upload->display_errors());
                    $this->session->set_flashdata('message', $text);
                    return redirect("mahasiswa/ta_d3/pendaftar_yudisium_ta_d3/".$id_trx);
            }
            else
            {
            ...
            $this->pendaftar_yudisium_ta_d3($id_trx);
        }
        ...
        if(($this->form_validation->run() == TRUE)){
            ...
            if(!empty($_FILES['berkas']['name'])){
                ...
                if ( ! $this->upload->do_upload('berkas'))
                {
                    $text = $this->alert->danger('Data failed Added. Info :'.$this->upload->display_errors());
                        $this->session->set_flashdata('message', $text);
                        return redirect("mahasiswa/ta_d3/add_bimbingan/".$nim);
                }
            }
                ...
                $data_masters=($this->Master_model->master_get(['nim' => $nim],'v_ta_d3_mhs_list'));
                $text = $this->alert->success('History Bimbingan add');
                    $this->session->set_flashdata('message', $text);
                    redirect("mahasiswa/ta_d3/dashboard/".$data_masters->periode); 
    ...
    public function add_bimbingan($nim){
        ...
        $data_master=$this->mahasiswa_form->add_bimbingan_d3($nim);
