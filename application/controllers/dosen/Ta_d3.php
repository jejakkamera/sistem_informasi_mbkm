...
class Ta_d3 extends CI_Controller {
    ...
    public function list_mahasiswa(){
        ...
        $data_master=$this->dosen_tabel->list_mahasiswa_ta_d3();
    ...
    public function json_list_mahasiswa_ta_d3(){
        $this->load->model('datatable/Dosen_dt', 'Dosen_dt');
        
        header('Content-Type: application/json');
        echo $this->Dosen_dt->json_list_mahasiswa_ta_d3();
    }

   

    public function dashboard_mhs($nim,$periode){
        $data_masters=($this->Master_model->master_get(['periode' => $periode,'nim' => $nim],'v_ta_d3_mhs_list'));
        if($data_masters){
            ...
            $history_sidang=$this->Master_model->master_result(['periode'=>$periode,'nim'=>$nim],'v_ta_d3_mhs_sidang');
    ...
    public function list_sidang(){
        ...
        $data_master=$this->dosen_tabel->list_sidang_ta_d3();
    ...
    public function json_list_jadwal_sidang_ta_d3(){
        $this->load->model('datatable/Dosen_dt', 'Dosen_dt');
        
        header('Content-Type: application/json');
        echo $this->Dosen_dt->json_list_jadwal_sidang_ta_d3();
    }

    public function upload_nilai_ta_d3($id_trx){
        $this->header();

        $data_file = $this->Master_model->master_get(['id_trx'=>$id_trx], 'v_ta_d3_mhs_sidang');

        $this->load->model('form/Dosen_f', 'dosen_from');
        $data_master=$this->dosen_from->upload_nilai_ta_d3($id_trx,$data_file);
    ...
    public function upload_nilai_ta_d3_action($id_trx){
        ...
        if ( ! $this->upload->do_upload('berkas'))
        {
            $text = $this->alert->danger('Data failed Added. Info :'.$this->upload->display_errors());
                $this->session->set_flashdata('message', $text);
                return redirect("dosen/ta_d3/upload_nilai_ta_d3/".$id_trx);
        }
        else
        {
            $magang_mhs_list=$this->Master_model->master_get(['id_trx'=>$id_trx],'v_ta_d3_mhs_sidang');
            if(!$magang_mhs_list){
                $text = $this->alert->warning('id trx registration not found');
                $this->session->set_flashdata('message', $text);
                redirect("dosen/ta_d3/list_sidang"); 
            }

            $magang_nilai=$this->Master_model->master_get(['id_trx_sidang'=>$id_trx,'email_penginput'=>$this->session->userdata('username')],'ta_d3_nilai');

            if($magang_nilai){
                $data=array(
                    'berkas'=>$this->upload->data("file_name"),
                );
                $this->Master_model->update_query(['id_trx_sidang'=>$id_trx,'email_penginput'=>$this->session->userdata('username')], $data, 'ta_d3_nilai');
                ...
                $this->Master_model->insert_query('ta_d3_nilai',$data);
            }

            $text = $this->alert->success('saved successfully');
            $this->session->set_flashdata('message', $text);
            redirect(site_url('dosen/ta_d3/list_sidang'));
        }
    }
        ...
        if(($this->form_validation->run() == TRUE)){
            ...
            if(!empty($_FILES['berkas']['name'])){
                ...
                if ( ! $this->upload->do_upload('berkas'))
                {
                    $text = $this->alert->danger('Data failed Added. Info :'.$this->upload->display_errors());
                        $this->session->set_flashdata('message', $text);
                        return redirect("dosen/ta_d3/add_bimbingan/".$nim);
                }
            }
                ...
                $text = $this->alert->success('History Bimbingan add');
                    $this->session->set_flashdata('message', $text);
                    redirect("dosen/ta_d3/dashboard_mhs/".$nim.'/'.$data_masters->periode); 
